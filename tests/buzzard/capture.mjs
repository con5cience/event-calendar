...
if (
  ...
  if (process.argv.length !== 2)
    throw Error("Usage: node tests/buzzard/capture.mjs");
  const r = await capture();
  const directory = mkdtempSync(join(tmpdir(), "event-calendar-buzzard-"));
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      validation: "Replay must compare both pages and passes",
    }),
    { mode: 0o644 },
  );
